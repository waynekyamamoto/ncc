...
# Phase 3 dual-build: ncc links src/type.c (chibicc-lineage),
# ncc-v2 links src/type_v2.c (spec-derived per docs/specs/03_type.md).
# Both share all other src/*.c and cc.h.  When validation passes
# (bootstrap fixed-point + torture + real programs) for ncc-v2, the
# swap-in step makes type_v2.c canonical and drops the dual-build.

COMMON_SRCS = $(filter-out src/type.c src/type_v2.c, $(wildcard src/*.c))
COMMON_OBJS = $(COMMON_SRCS:.c=.o)

ncc: $(COMMON_OBJS) src/type.o
	$(CC) $(LDFLAGS) -o $@ $^

ncc-v2: $(COMMON_OBJS) src/type_v2.o
	$(CC) $(LDFLAGS) -o $@ $^
