...
static bool is_redundant_mov(const char *line) {
  ...
  // Only X-form `mov xN, xN` is a true no-op. The W-form `mov wN, wN`
  // zero-extends the low 32 bits to 64 (clearing the upper 32), which
  // is the canonical narrowing of an unsigned int to itself; dropping
  // it loses the implicit clear and breaks unsigned->long widening
  // (gcc torture 20140622-1: `(long)(p + a) - (long)p` for unsigned p,a).
  if (r1[0] != 'x' && r1[0] != 'X') return false;
